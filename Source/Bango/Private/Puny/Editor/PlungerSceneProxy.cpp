...
void BuildPunyBoxVerts(const FVector3f& StaticOffset, TArray<FDynamicMeshVertex>& OutVerts, TArray<uint32>& OutIndices);
void BuildPunyCylinderVerts(const FVector& Base, const FVector& XAxis, const FVector& YAxis, const FVector& ZAxis, double Radius, double HalfHeight, uint32 Sides, TArray<FDynamicMeshVertex>& OutVerts, TArray<uint32>& OutIndices);


// FPunyPlungerSceneProxy
//=================================================================================================
...
void FPunyPlungerSceneProxy::PreparePlungerMesh(const FPlungerMeshConstructionData& MeshData)
{
	...
	BuildPunyBoxVerts(BoxOrigin, OutVerts_HandleUp, MeshData.IndexBuffer->Indices);
	BuildPunyCylinderVerts(StemOffset, FVector::ForwardVector, FVector::RightVector, FVector::UpVector, MeshData.StemRadius, 0.5f * MeshData.StemHeight, 8, OutVerts_HandleUp, MeshData.IndexBuffer->Indices);
	BuildPunyCylinderVerts(HandleOffset, FVector::ForwardVector, FVector::UpVector, FVector::LeftVector, MeshData.HandleRadius, 0.5f * MeshData.HandleWidth, 8, OutVerts_HandleUp, MeshData.IndexBuffer->Indices);
...
FPrimitiveViewRelevance FPunyPlungerSceneProxy::GetViewRelevance(const FSceneView* View) const
{
#if WITH_EDITOR
	...
	const bool bProxyVisible = IsShown(View) && (UPunyEvent::PunyEventsShowFlag.IsEnabled(View->Family->EngineShowFlags));
...
void FPunyPlungerSceneProxy::OnTransformChanged()
{
